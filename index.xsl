...
<head>
    ...
    <style type="text/css">
        ...
        #content { margin: 10px auto; max-width: 960px; }
        .expl { margin: 10px 3px; line-height: 1.3em; }
        .expl a, .expl a:visited, footer a, footer a:visited { color: #da3114; font-weight: bold; }
        a { color: #000; text-decoration: none; }
        a:visited { color: #777; }
        a:hover { text-decoration: underline; }
        td { font-size:11px; }
        th { text-align:left; padding: 5px 20px 5px 5px; font-size: 12px; }
        thead th { border-bottom: 1px solid #dedede; cursor: pointer; }
        footer { margin:20px auto; text-align:left; max-width:100%; }
...
<body>
    <div id="content">
        <h2>XML Sitemap</h2>
        <p class="expl">This is styled XML sitemap, sorted by update date.</p>
            ...
            <tbody>
                <xsl:for-each select="s:urlset/s:url">
                    <tr>
                        ...
                        <td><xsl:value-of select="concat(substring(s:lastmod, 0, 11), ' ', substring(s:lastmod, 12, 5))"/></td>
